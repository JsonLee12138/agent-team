...
**Goal:** Rewrite agent-team CLI from Python to Go, producing a single binary (`agent-team`) distributable via Homebrew.
...
**Reference:** Archived design doc at `docs/plans/archive/2026-02-24-solo-ops-go-rewrite-design.md`
...
func TestBuildLaunchCmd(t *testing.T) {
	tests := []struct {
		...
		{"claude", "claude-sonnet-4-6", "claude --dangerously-skip-permissions --model claude-sonnet-4-6"},
...
func TestResolveModel(t *testing.T) {
	if got := ResolveModel("gpt-5", "sonnet"); got != "gpt-5" {
		t.Errorf("ResolveModel(cli, config) = %q, want gpt-5", got)
	}
	if got := ResolveModel("", "sonnet"); got != "sonnet" {
		t.Errorf("ResolveModel('', config) = %q, want sonnet", got)
	}
...
func TestValidateProvider(t *testing.T) {
	if err := ValidateProvider("claude"); err != nil {
		t.Errorf("ValidateProvider(claude) = %v", err)
	}
	if err := ValidateProvider(""); err != nil {
		t.Errorf("ValidateProvider('') should be nil for empty (default)")
	}
	if err := ValidateProvider("invalid"); err == nil {
		t.Error("ValidateProvider(invalid) should return error")
	}
}
...
func TestGenerateClaudeMDWithoutPrompt(t *testing.T) {
	dir := t.TempDir()
	wtPath := filepath.Join(dir, ".worktrees", "dev")
	...
	err := GenerateClaudeMD(wtPath, "dev", dir)
	if err != nil {
		t.Fatalf("GenerateClaudeMD without prompt: %v", err)
	}

	data, _ := os.ReadFile(filepath.Join(wtPath, "CLAUDE.md"))
	content := string(data)
	if !strings.Contains(content, "Development Environment") {
		t.Error("CLAUDE.md should still contain worktree context even without prompt.md")
	}
}
...
Run: `go test ./internal/ -run "TestFindWtBase|TestListRoles|TestBuildLaunchCmd|TestResolveModel|TestValidateProvider|TestSlugify|TestGenerateClaudeMD|TestPromptMD" -v`
...
// ResolveModel returns CLI model if set, otherwise falls back to config model.
func ResolveModel(cliModel, configModel string) string {
	if cliModel != "" {
		return cliModel
	}
	return configModel
...
// ValidateProvider checks that the provider string is supported. Empty is OK (uses default).
func ValidateProvider(provider string) error {
	if provider == "" {
		return nil
	}
	if !SupportedProviders[provider] {
		return fmt.Errorf("unsupported provider %q (supported: claude, codex, opencode)", provider)
	}
	return nil
}
...
func GenerateClaudeMD(wtPath, name, root string) error {
	...
	// Write prompt.md content if it exists
	...
	// Always append worktree context
...
git commit -m "feat: add role discovery, path helpers, model resolution, and templates"
```
...
**Step 1: Write root command, App struct, and main.go**
...
// Shared across all cmd tests via the same package.
...
func (m *MockBackend) PaneSend(_ string, t string) error {
	m.SentTexts = append(m.SentTexts, t)
	return nil
}
func (m *MockBackend) SpawnPane(_ string) (string, error) { return m.SpawnedID, nil }
func (m *MockBackend) KillPane(id string) error {
	m.KilledIDs = append(m.KilledIDs, id)
	return nil
}
...
func TestRunDeleteKillsRunningPane(t *testing.T) {
	...
	found := false
	for _, id := range mock.KilledIDs {
		if id == "123" {
			found = true
		}
	}
	if !found {
		t.Error("expected KillPane to be called with pane 123")
	}
}
```
...
func TestRunOpenUsesConfigModel(t *testing.T) {
	app, dir := initTestApp(t)
	mock := &MockBackend{SpawnedID: "66"}
	app.Session = mock
	app.RunCreate("modeled")

	// Set default_model in config
	configPath := filepath.Join(dir, ".worktrees", "modeled", "agents", "teams", "modeled", "config.yaml")
	cfg, _ := internal.LoadRoleConfig(configPath)
	...
	if !strings.Contains(mock.SentTexts[0], "--model claude-sonnet-4-6") {
		t.Errorf("sent = %q, want --model claude-sonnet-4-6 from config", mock.SentTexts[0])
	}
}

func TestRunOpenCLIModelOverridesConfig(t *testing.T) {
	app, dir := initTestApp(t)
	mock := &MockBackend{SpawnedID: "67"}
	app.Session = mock
	app.RunCreate("override")

	configPath := filepath.Join(dir, ".worktrees", "override", "agents", "teams", "override", "config.yaml")
	cfg, _ := internal.LoadRoleConfig(configPath)
	cfg.DefaultModel = "config-model"
	cfg.Save(configPath)

	app.RunOpen("override", "", "cli-model")

	if len(mock.SentTexts) == 0 {
		t.Fatal("no command sent")
	}
	if !strings.Contains(mock.SentTexts[0], "--model cli-model") {
		t.Errorf("sent = %q, want --model cli-model (CLI overrides config)", mock.SentTexts[0])
	}
}

func TestRunOpenInvalidProvider(t *testing.T) {
	app, _ := initTestApp(t)
	mock := &MockBackend{SpawnedID: "99"}
	app.Session = mock
	app.RunCreate("prov")

	err := app.RunOpen("prov", "invalid-provider", "")
	if err == nil {
		t.Error("expected error for invalid provider")
	}
}

func TestRunOpenAll(t *testing.T) {
	app, _ := initTestApp(t)
	mock := &MockBackend{SpawnedID: "44"}
	app.Session = mock
	app.RunCreate("alpha")
	app.RunCreate("beta")
	...
	err := app.RunOpenAll("", "")
	if err != nil {
		t.Fatalf("RunOpenAll: %v", err)
	}

	// Should have sent commands to both roles
	if len(mock.SentTexts) < 2 {
		t.Errorf("expected at least 2 pane sends, got %d", len(mock.SentTexts))
	}
}
```
...
func (a *App) RunOpen(name, provider, model string) error {
	...
	if err := internal.ValidateProvider(provider); err != nil {
		return err
	}

	// Resolve model: CLI > config
	resolvedModel := internal.ResolveModel(model, cfg.DefaultModel)
	...
	if paneID == "" {
		return fmt.Errorf("failed to open session for '%s': empty pane ID returned", name)
	}
	...
	launchCmd := internal.BuildLaunchCmd(provider, resolvedModel)
...
func (a *App) RunAssign(name, task, provider, model string) error {
	...
	if !a.Session.PaneAlive(cfg.PaneID) {
		...
		// Resolve model from config for auto-open
		resolvedModel := internal.ResolveModel(model, cfg.DefaultModel)
		if err := a.RunOpen(name, provider, resolvedModel); err != nil {
			return err
		}
		cfg, err = internal.LoadRoleConfig(configPath)
		if err != nil {
...
## Changes from Archived Plan

This plan fixes the following issues found in the archived `2026-02-24-solo-ops-go-impl.md`:

1. **All imports are complete in code blocks** — no more "Note: add X to imports" prose
2. **`cmd/root.go`**: `"context"` import included; `GetApp` is nil-safe
3. **`cmd/commands_test.go`**: `"os"` and `"os/exec"` imports included in code block
4. **`cmd/delete_test.go`**: `"github.com/leeforge/agent-team/internal"` import included
5. **`ResolveModel` added** to `internal/role.go` — CLI model > config `default_model`
6. **`ValidateProvider` added** — rejects unsupported provider strings
7. **`RunOpen` uses model resolution** from `cfg.DefaultModel` as fallback
8. **`RunAssign` resolves model** from config when auto-opening offline roles
9. **`GenerateClaudeMD` generates worktree context** even when `prompt.md` is missing
10. **`RunOpen` nil-error fix** — separate checks for `err != nil` and empty `paneID`
11. **`MockBackend.KilledIDs`** tracks kill calls for proper test assertions
12. **Test for `RunOpenAll`** added
13. **Test for `BuildLaunchCmd` with claude+model** added
14. **Test for config model resolution** (`TestRunOpenUsesConfigModel`, `TestRunOpenCLIModelOverridesConfig`)
15. **Test for invalid provider** (`TestRunOpenInvalidProvider`)
16. **Test for `GenerateClaudeMD` without prompt.md** (`TestGenerateClaudeMDWithoutPrompt`)
...
- **Environment variable**: `AGENT_TEAM_BACKEND` (replaces Python's `SOLO_OPS_BACKEND`)
