...
│
├── .claude-plugin/                      # Claude Code 插件（现有，重构 hooks）
│   ├── plugin.json
│   ├── marketplace.json
│   └── hooks/
│       └── hooks.json                   # Claude 格式 hooks → agent-team hook <event>
│
├── hooks/                               # Gemini 扩展读取此目录
│   └── hooks.json                       # Gemini 格式 hooks → agent-team hook <event>
│
├── adapters/
...
│
├── scripts/                             # 现有 shell 脚本（迁移期保留）
└── settings.json                        # 插件设置
```

### 2. Repo Root 同时作为 Claude 插件和 Gemini 扩展

- Claude 读 `.claude-plugin/hooks/hooks.json`（Claude 格式）
- Gemini 读 `hooks/hooks.json`（Gemini 格式）
- 两份 hooks.json 格式不同，但最终都调用 `agent-team hook <event>`
- 无文件冲突，各读各的目录
...
`.claude-plugin/hooks/hooks.json`（Claude 格式）:
...
├── brainstorming_gate.go → 依赖 internal/task_store.go (查找 active change)
├── post_edit_check.go    → 依赖 internal/task_store.go + role.go
├── stop_guard.go         → 依赖 internal/task_store.go (未完成 changes)
├── task_completed.go     → 依赖 internal/task_lifecycle.go (自动验证+归档)
├── subagent_context.go   → 依赖 internal/role.go (读 CLAUDE.md/GEMINI.md)
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| `gemini-extension.json` 在 repo root 与 Claude 插件共存 | 低 | 文件无冲突，各读各的目录 |
...
| 现有 shell scripts 迁移到 Go 的行为差异 | 中 | 并行保留旧 scripts，逐步切换 |
...
| 阶段 | 内容 | 前置依赖 |
|------|------|---------|
| **Phase 0** | .tasks/ TDD 任务系统开发 | 无（独立 brainstorming 文档已批准） |
| **Phase 1** | Go hook 子命令框架 + `session-start`（角色注入） | Phase 0 |
| **Phase 2** | Gemini 扩展清单 + hooks 配置 + GEMINI.md | Phase 1 |
| **Phase 3** | OpenCode npm 插件 (opencode-agent-team) | Phase 1 |
| **Phase 4** | 迁移剩余 5 个 hooks 到 Go（依赖 .tasks/） | Phase 0 + Phase 1 |
| **Phase 5** | 重构 Claude 插件 hooks 指向 Go binary | Phase 4 |
