...
- `draft`: change exists, but implementation has not started or the proposal is still being reviewed.
- `in_progress`: implementation or document updates are actively being produced.
- `verify_pending`: implementation is complete and verification is the next required action.
- `completed`: verification is done and task-scoped changes are committed.
- `archived`: archive command succeeded and the completion message was sent to main.
