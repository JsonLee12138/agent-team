...
- MUST state whether the issue is deterministic, flaky, or currently unreproducible.
...
- MUST record the smallest set of files, functions, or commands involved in the failure.

### 3. Isolate the Cause

- MUST reduce the issue to the smallest reproducible case.
- MUST change one variable at a time when testing a hypothesis.
- MAY add temporary logging or probes, but MUST remove them before commit.
...
- MUST confirm that the fix does not hide the original error behind fallback behavior.

## Prohibited Shortcuts

- NEVER patch blindly without a confirmed reproduction or evidence trail.
- NEVER silence errors, skip assertions, or weaken checks only to make tests pass.
- NEVER leave temporary debug code, TODO-only fixes, or unverifiable assumptions in the final change.
